...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '02619a05-e945-408d-8672-6f8bdc5df74e'; // ID de Laksmispa
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Laksmispa';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '58274908';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'Mariasera97@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Laksmispa!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'laksmispa';
};
...
console.log('✅ config-negocio.js listo para Laksmispa');
